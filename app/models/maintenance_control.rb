class MaintenanceControl < ApplicationRecord
  ...
  def self.enviar_json(current_user,order)
    sorder = order.service_order
    if sorder.factura.attached?
      doc = Nokogiri::XML(sorder.factura.download)
      archivo = Hash.from_xml(doc.to_s)
      hash = archivo["Comprobante"]
    ...
    # byebug
      ...
      imponible = (impuestos_fact.to_f / iva_fact)
      ...
      if (iva_fact * 100).to_i == 16
        hash_poliza["tasaFiscal"] = "IVA15GAS"
      else
        hash_poliza["tasaFiscal"] = "IVA8GAS"
      end
  ...
  #---------------------------------------------------------------------------------------
      ...
      imponible = (order.impuestos.to_f / iva_fact)
      imponible_global = ((imponible) * 100).to_i
  ...
  # comentar de aquí a...
      ...
      @mensaje_error  = ["La factura no ha sido anexada. Folio: #{order.folio}"]
      return [@mensaje_error,false] 
    end
  end
